...
data_filename = 'Graaf_Fig5p6_ver4p0';
...
b1max_excitation_beta1 = (alpha_excitation / 360) / (gamma * sum(fb) * dt) * 1e2; % [uT]
fprintf(sprintf('b1max_excitation = %f\n', b1max_excitation_beta1));

b1max_inversion_beta1 = (alpha_inversion / 360) / (gamma * sum(fb) * dt) * 1e2; % [uT]
fprintf(sprintf('b1max_inversion = %f\n', b1max_inversion_beta1));
...
b1_sinc_excitation = b1max_excitation_beta1 * fb; % [uT]
b1_sinc_inversion = b1max_inversion_beta1 * fb; % [uT]
...
plot(Delta_Hz*1e-3, mx_sinc_excitation, 'r', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, my_sinc_excitation, 'b', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, abs(mxy_sinc_excitation), 'k', 'LineWidth', LineWidth);
legend('M_x','M_y','M_{xy}','Location','Southeast'); hold off;
axis([-1e1 1e1 -1.1 1.1]); xlabel('Frequency (kHz)'); ylabel('M_{xy}/M_0');

% Display 5.6(C)
subplot(2,3,3), plot(Delta_Hz*1e-3, mz_sinc_inversion, 'k', 'LineWidth', LineWidth); hold on;
plot(Delta_Hz*1e-3, zeros(length(Delta_Hz),1), 'k--'); hold off; set(gca, 'Box', 'off');
xlabel('Frequency (kHz)'); ylabel('M_z/M_0');

%%
% Robin_de_Graaf figure 5.6 (D)
beta = 1;
gaussian_func_beta = exp(-beta.*(2/tau*(t-tau/2)).^2);
b1max_excitation_beta = (alpha_excitation / 360) / (gamma * sum(gaussian_func_beta) * dt) * 1e2; % [uT]

b1_gaussian_excitation_beta = b1max_excitation_beta * exp(-beta.*(2/tau*(t-tau/2)).^2);

beta1 = -log(b1_gaussian_excitation_beta(end)*1e-2/b1max_excitation_beta); % 1% gauss
beta2 = -log(b1_gaussian_excitation_beta(end)*1e-1/b1max_excitation_beta); % 10% gauss

gaussian_func_beta1 = exp(-beta1.*(2/tau*(t-tau/2)).^2);
gaussian_func_beta2 = exp(-beta2.*(2/tau*(t-tau/2)).^2);

b1max_excitation_beta = (alpha_excitation / 360) / (gamma * sum(gaussian_func_beta) * dt) * 1e2; % [uT]

b1_gaussian_excitation_beta1 = b1max_excitation_beta * exp(-beta1.*(2/tau*(t-tau/2)).^2);
b1_gaussian_excitation_beta2 = b1max_excitation_beta * exp(-beta2.*(2/tau*(t-tau/2)).^2);

b1max_inversion_beta1 = (alpha_inversion / 360) / (gamma * sum(gaussian_func_beta1) * dt) * 1e2; % [uT]
b1max_inversion_beta2 = (alpha_inversion / 360) / (gamma * sum(gaussian_func_beta2) * dt) * 1e2; % [uT]
b1_gaussian_inversion_beta1 = b1max_inversion_beta1 * exp(-beta1.*(2/tau*(t-tau/2)).^2);
b1_gaussian_inversion_beta2 = b1max_inversion_beta2 * exp(-beta2.*(2/tau*(t-tau/2)).^2);
...
%--------------------------------------------------------------------------
% Display 5.6 (D)
subplot(2,3,4), plot(t*1e3, b1_gaussian_excitation_beta1, 'Color', gray, 'LineWidth', LineWidth);
hold on; plot(t*1e3, b1_gaussian_excitation_beta2, 'Color', black, 'LineWidth', LineWidth); hold off;
set(gca, 'Box', 'off'); xlabel('Time (ms)'); ylabel('RF amplitude');

% Display 5.6 (E)
subplot(2,3,5), hold on;
plot(Delta_Hz*1e-3, mx_gaussian_excitation_beta2, 'r', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, my_gaussian_excitation_beta2, 'b', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, abs(mxy_gaussian_excitation_beta2), 'k', 'LineWidth', LineWidth);
legend('M_x','M_y','M_{xy}','Location','Southeast'); hold off;
axis([-1e1 1e1 -1.1 1.1]); xlabel('Frequency (kHz)'); ylabel('M_{xy}/M_0');

% Display 5.6 (F)
subplot(2,3,6), plot(Delta_Hz*1e-3, mz_gaussian_inversion_beta1, 'Color', gray, 'LineWidth', LineWidth); hold on;
plot(Delta_Hz*1e-3, mz_gaussian_inversion_beta2, 'Color', black, 'LineWidth', LineWidth); 
plot(Delta_Hz*1e-3, zeros(length(Delta_Hz*1e-3),1), 'k--'); hold off; 
set(gca, 'Box', 'off'); xlabel('Frequency (kHz)'); ylabel('M_z/M_0');
legend('1% gauss','10% gauss','Location','Southeast');

% save figure
export_fig(fullfile(output_dir, data_filename), '-tif', '-m2');
close;
