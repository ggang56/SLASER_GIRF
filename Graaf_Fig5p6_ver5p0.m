...
gaussian_func = exp(-(2/tau*(t-tau/2)).^2);
...
scale_factor = max(mx_gaussian_excitation_beta1);
mx_gaussian_excitation_beta1 = mx_gaussian_excitation_beta1/scale_factor;

scale_factor = max(my_gaussian_excitation_beta1);
my_gaussian_excitation_beta1 = my_gaussian_excitation_beta1/scale_factor;
...
%     bloch(b1_gaussian_excitation_beta2*1e-2, zeros(nt,1), dt, 1e2, 1e2, Delta_Hz, 0, 0, mx0, my0, mz0);

% scale_factor = max(abs(mx_gaussian_excitation_beta1));
% mx_gaussian_excitation_beta = mx_gaussian_excitation_beta1 / scale_factor;
% 
% scale_factor = max(abs(my_gaussian_excitation_beta1));
% my_gaussian_excitation_beta1 = my_gaussian_excitation_beta1 / scale_factor;
...
[mx_gaussian_inversion_beta1,my_gaussian_inversion_beta1,mz_gaussian_inversion_beta1] = ...
    bloch(b1_gaussian_inversion_beta1*1e-2, zeros(nt,1), dt, 1e2, 1e2, Delta_Hz, 0, 0, mx0, my0, mz0);
...
[mx_gaussian_inversion_beta2,my_gaussian_inversion_beta2,mz_gaussian_inversion_beta2] = ...
    bloch(b1_gaussian_inversion_beta2*1e-2, zeros(nt,1), dt, 1e2, 1e2, Delta_Hz, 0, 0, mx0, my0, mz0);
...
plot(Delta_Hz*1e-3, mx_gaussian_excitation_beta1, 'r', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, my_gaussian_excitation_beta1, 'b', 'LineWidth', LineWidth);
plot(Delta_Hz*1e-3, abs(mxy_gaussian_excitation_beta1), 'k', 'LineWidth', LineWidth);
legend('M_x','M_y','M_{xy}','Location','Southeast'); hold off;
axis([-1e1 1e1 -1.1 1.1]); xlabel('Frequency (kHz)'); ylabel('M_{xy}/M_0');
